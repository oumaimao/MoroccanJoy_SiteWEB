...
$sqlstate=$connection->prepare('SELECT A_email FROM adminee WHERE admin_id=?');
$id=$_GET['id'];
$sqlstate->execute([$id]);
...
if(isset($_POST['modifier'])){
	...
	if(!empty($email)){
		$sql = 'UPDATE adminee SET 
								A_email = :email 
								WHERE admin_id = :A_id';
		$statement = $connection->prepare($sql); 

		if($statement->execute([':email'=>$email, ':A_id'=>$id	])){
			$message = '<div class="alert alert-success" role="alert">
								Donnée modifier avec succès
							</div>';
		...
		<div class="checkout-block">
			<div class="main-card">
					...
					<div class="row">
						<div class="col-lg-6 col-md-12">
							<div class="form-group mt-4">
								...
								<input class="form-control h_50" name="email" type="text" placeholder="" value="<?php echo $data['A_email']; ?>">																								
							</div>
						</div>		
					</div>
				</div>
				...
				<div class="container step-footer step-tab-pager mt-4">									
					...
					<a href="Admin.php"><button type="button" class="co-main-btn min-width btn-hover h_40">Annuler</button></a>
				</div>
		</div>
